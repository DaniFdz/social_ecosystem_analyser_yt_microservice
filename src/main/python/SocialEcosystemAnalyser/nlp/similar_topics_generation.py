...
def generate_topics(topic: str, api_key: str):
    try:
        ...
        response = co.generate(
            model='command',
            prompt=f'For each topic I want 5 similar topics:\n\n\"Animals\": \"zoo,mamals,elepahnts,dogs,pet\";\n\"{topic}\":',
            max_tokens=300,
            temperature=1.2, # The range is 0-2, with 0 being the most conservative and 2 being the most creative.
            k=0,
            stop_sequences=[";"],
        ...
        raise SocialEcosystemAnalyserException(
            MessageExceptions.COHERE_API_ERROR
        ) from e
